 ...
 * <br/><br/>
 * This class is an implementation of a <b>"High Pass Filter"</b>.
 * It extends the <a href="StandardKPIFilter.html">StandardKPIFilter</a> abstract class by 
 * adding some specific properties that characterize a <i>"High Pass Filter"</i> and providing an 
 ...
 *  <li> "cutoff" (is a double value), that is the lower limit value used to determine the accepted values, 
 *  in fact the values that stay under this limit will be discarded. 
 * </ul
 * 
 * Obviously, these parameters must be provided (to the ECoWareProcessor) by the XML configuration file.
 * 
 * As can be easily understood, a high pass filter is a filter that accepts all values that are above the 
 * lower limit and rejects all other values. So the "cutoff" limit is used to determine the accepted 
 ...
 *
 */
public class HPFilter extends StandardKPIFilter {
	...
	/**
	 * Constructs a new HPFilter using the specified XML element. The bus server name (that is 
	 ...
	 */
	public HPFilter(Element xmlElement, String busServer, Configuration esperConfiguration) {
	...
	@Override
	/**
	 * This method actually starts the "High Pass" filter.<br/>
	 */
	public void launch() {
		
		System.out.println("---");
		System.out.println("Initializing high-pass filter...");
